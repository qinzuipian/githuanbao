...
layui.define("layer", function(e) {
	...
	p.prototype.config = {
		...
		var e = this,
			...
			o = i.elem.next();
		(o.hasClass(u) || o.hasClass(c)) && o.remove(), a.ie && a.ie < 10 && i.elem.wrap(
		...
		var e = this,
			...
			a = t(['<form target="' + f + '" class="' + c +
				'" method="post" key="set-mine" enctype="multipart/form-data" action="' + i.url + '">', "</form>"
			].join(""));
		t("#" + f)[0] || t("body").append(n), i.elem.next().hasClass(c) || (e.elemFile.wrap(a), i.elem.next("." + c).append(
			function() {
				var e = [];
		...
		var n, o = this,
			...
			y = function() {
				if ("choose" !== i && !l.auto || (l.choose && l.choose(g), "choose" !== i)) return l.before && l.before(g), a.ie ?
					a.ie > 9 ? u() : c() : void u()
			};
		...
		var e = this,
			...
			l = function(t, n) {
				...
				a.next().hasClass(s) && a.next().remove(), e.upload(null, "choose"), e.isFile() || i.choose || a.after(
			...
			if (layui.each(d, function(e, l) {
					...
					if (r.removeClass(o), layui.each(c, function(e, t) {
							...
							if (a[t]) {
								...
								if (f = f || a[t][1], "required" === t && (f = r.attr("lay-reqText") || f), c) return "tips" === u ? i.tips(
									f,
									function() {
...
layui.define("form", function(e) {
	...
	g.prototype.config = {
		...
		var e = this,
			a = e.config,
			n = i('<div class="layui-tree' + (a.showCheckbox ? " layui-form" : "") + (a.showLine ? " layui-tree-line" : "") +
				'" lay-filter="LAY-tree-' + e.index + '"></div>');
		e.tree(n);
		var r = a.elem = i(a.elem);
		if (r[0]) {
			if (a.showSearch && n.prepend(
					'<input type="text" class="layui-input layui-tree-search" placeholder="请输入关键字进行过滤">'), e.key = a.id || e.index,
				e.elem = n, e.elemNone = i('<div class="layui-tree-emptyText">' + a.text.none + "</div>"), r.html(e.elem), 0 ==
				e.elem.find(".layui-tree-set").length) return e.elem.append(e.elemNone);
			a.drag && e.drag(), a.showCheckbox && e.renderForm("checkbox"), e.elem.find(".layui-tree-set").each(function() {
				...
				e.parent(".layui-tree-pack")[0] || e.addClass("layui-tree-setHide"), !e.next()[0] && e.parents(
					".layui-tree-pack").eq(1).hasClass("layui-tree-lineExtend") && e.addClass(C), e.next()[0] || e.parents(
					".layui-tree-set").eq(0).next()[0] || e.addClass(C)
			}), e.events()
		}
		...
		layui.each(l, function(a, l) {
			var c = l.children && l.children.length > 0,
				...
				h = i(['<div data-id="' + l.id + '" class="layui-tree-set' + (l.spread ? " layui-tree-spread" : "") + (l.checked ?
						" layui-tree-checkedFirst" : "") + '">', "<div " + (r.drag && !l.fixed ? 'draggable="true"' : "") +
					' class="layui-tree-entry">', '<div class="layui-tree-main">',
					function() {
						return r.showLine ? c ? '<span class="layui-tree-iconClick layui-tree-icon"><i class="layui-icon ' + (l.spread ?
							...
							'<span class="layui-tree-iconClick"><i class="layui-tree-iconArrow ' + (c ? "" : t) + '"></i></span>'
					}(),
					function() {
						return r.showCheckbox ? '<input type="checkbox" name="layuiTreeCheck" lay-skin="primary" ' + (l.disabled ?
							"disabled" : "") + '  value="' + l.id + '">' : ""
					}(),
					function() {
						return r.isJump && l.href ? '<a href="' + l.href + '" target="_blank" class="' + p + '">' + (l.title || l.label ||
							r.text.defaultNodeName) + "</a>" : '<span class="' + p + (l.disabled ? " " + d : "") + '">' + (l.title ||
							l.label || r.text.defaultNodeName) + "</span>"
					}(), "</div>",
					function() {
						if (!r.edit) return "";
						var e = {
						...
						return r.edit === !0 && (r.edit = ["update", "del"]), "object" == typeof r.edit ? (layui.each(r.edit,
							function(a, n) {
								i.push(e[n] || "")
							}), i.join("") + "</div>") : void 0
					}(), "</div></div>"
				].join(""));
			c && (h.append(o), n.tree(o, l.children)), e.append(h), h.prev("." + s)[0] && h.prev().children(
				".layui-tree-pack").addClass("layui-tree-showLine"), c || h.parent(".layui-tree-pack").addClass(
				"layui-tree-lineExtend"), n.spread(h, l), r.showCheckbox && n.checkClick(h, l), r.edit && n.operate(h, l)
		})
		...
		k.on("click", function(i) {
			var a = e.children("." + y),
				n = k.children(".layui-icon")[0] ? k.children(".layui-icon") : k.find(".layui-tree-icon").children(
					".layui-icon");
			if (a[0]) {
				if (e.hasClass(v)) e.removeClass(v), a.slideUp(200), n.removeClass(h).addClass(o);
					...
					var l = e.siblings("." + s);
					l.removeClass(v), l.children("." + y).slideUp(200), l.find(".layui-tree-icon").children(".layui-icon").removeClass(
						h).addClass(o)
				}
			} else g = "normal"
		}), m.on("click", function() {
			var n = i(this);
			n.hasClass(d) || (g = e.hasClass(v) ? r.onlyIconControl ? "open" : "close" : r.onlyIconControl ? "close" :
				"open", r.click && r.click({
					elem: e,
					state: g,
					data: a
				}))
		...
		l.children(".layui-tree-btnGroup").on("click", ".layui-icon", function(l) {
			...
			if ("add" == f) {
				g[0] || (r.showLine ? (d.find("." + c).addClass("layui-tree-icon"), d.find("." + c).children(".layui-icon").addClass(
					o).removeClass("layui-icon-file")) : d.find(".layui-tree-iconArrow").removeClass(t), e.append(
				...
				if (w.title = r.text.defaultNodeName, w.id = b, n.tree(e.children("." + y), [w]), r.showLine)
					if (g[0]) g.hasClass(k) || g.addClass(k), e.find("." + y).each(function() {
					...
					}), g.children("." + s).last().prev().hasClass(C) ? g.children("." + s).last().prev().removeClass(C) : g.children(
						"." + s).last().removeClass(C), !e.parent("." + y)[0] && e.next()[0] && g.children("." + s).last().removeClass(
						C);
					else {
						var T = e.siblings("." + s),
							L = 1,
						...
						}), 1 == L ? (T.children("." + y).addClass(m), T.children("." + y).children("." + s).removeClass(C), e.children(
								"." + y).addClass(m), N.removeClass(k), N.children("." + s).last().children("." + y).children("." + s).last()
							.addClass(C)) : e.children("." + y).children("." + s).addClass(C)
				...
				d.children("." + p).html(""), d.append('<input type="text" class="layui-tree-editInput">'), d.children(
					".layui-tree-editInput").val(q).focus();
				var F = function(e) {
					var i = e.val().trim();
					i = i ? i : r.text.defaultNodeName, e.remove(), d.children("." + p).html(i), x.data.title = i, r.operate &&
						r.operate(x)
				};
				...
				if (r.operate && r.operate(x), x.status = "remove", !e.prev("." + s)[0] && !e.next("." + s)[0] && !e.parent(
				...
				if (e.siblings("." + s).children("." + u)[0]) {
					...
					if (r.showLine) {
						...
						layui.each(T, function(e, a) {
								i(a).children("." + y)[0] || (L = 0)
							}), 1 == L ? (g[0] || (N.removeClass(k), T.children("." + y).addClass(m), T.children("." + y).children("." +
									s).removeClass(C)), e.next()[0] ? N.children("." + s).last().children("." + y).children("." + s).last().addClass(
									C) : e.prev().children("." + y).children("." + s).last().addClass(C), e.next()[0] || e.parents("." + s)[1] ||
								e.parents("." + s).eq(0).next()[0] || e.prev("." + s).addClass(C)) : !e.next()[0] && e.hasClass(C) && e.prev()
							.addClass(C)
					}
				} else {
					...
					if (r.showLine) {
						...
						H.find("." + c).removeClass("layui-tree-icon"), H.find("." + c).children(".layui-icon").removeClass(h);
		...
		e.elem.on("dragstart", "." + u, function() {
			...
			var S = function(n) {
				if (T || L || e.elem.children("." + s).last().children("." + y).children("." + s).last().addClass(C), !T)
					return void d.removeClass("layui-tree-setHide");
				if (1 == H) a.showLine ? (n.find("." + c).removeClass("layui-tree-icon"), n.find("." + c).children(
					// ".layui-icon").removeClass(h).addClass("layui-icon-file"), q.addClass(k), q.children("." + s).children("." +
					".layui-icon").removeClass(h), q.addClass(k), q.children("." + s).children("." +
					y).each(function() {
					i(this).children("." + s).last().addClass(C)
					...
					if (a.showLine) {
						...
						layui.each(I, function(e, a) {
							...
							}), 1 == r ? (d.children("." + y)[0] || (N.removeClass(k), I.children("." + y).addClass(m), I.children("." +
									y).children("." + s).removeClass(C)), N.children("." + s).last().children("." + y).children("." + s).last()
								.addClass(C), L || n.parents("." + s)[0] || n.next()[0] || N.children("." + s).last().addClass(C)) : !L &&
							d.hasClass(C) && N.children("." + s).last().addClass(C)
					}
					if (a.showCheckbox) {
						var l = function(a) {
							if (a) {
			...
			g.each(function() {
				if (0 != i(this).height()) {
					...
					if (r > n && r < n + 15) {
						if (i(this).children("." + y)[0] || (a.showLine ? (i(this).find("." + c).eq(0).addClass("layui-tree-icon"),
								i(this).find("." + c).eq(0).children(".layui-icon").addClass(o).removeClass("layui-icon-file")) : i(
								this).find(".layui-tree-iconArrow").removeClass(t), i(this).append(
								'<div class="layui-tree-pack"></div>')), i(this).children("." + y).append(d), S(A), a.showLine) {
							...
							if (d.children("." + y).children("." + s).last().addClass(C), 1 == l.length) {
								...
								}), 1 == v ? (h.children("." + y).addClass(m), h.children("." + y).children("." + s).removeClass(C), i(
									this).children("." + y).addClass(m), g.removeClass(k), g.children("." + s).last().children("." + y).children(
									"." + s).last().addClass(C).removeClass("layui-tree-setHide")) : i(this).children("." + y).children(
									"." + s).addClass(C).removeClass("layui-tree-setHide")
							} else d.prev("." + s).hasClass(C) ? (d.prev("." + s).removeClass(C), d.addClass(C)) : (d.removeClass(
								"layui-tree-setLineShort layui-tree-setHide"), d.children("." + y)[0] ? d.prev("." + s).children("." +
								y).children("." + s).last().removeClass(C) : d.siblings("." + s).find("." + y).each(function() {
								i(this).children("." + s).last().addClass(C)
					...
					if (r < n) {
						if (i(this).before(d), S(A), a.showLine) {
							...
							if (b[0]) {
								...
								layui.each(h, function(e, a) {
									i(a).children("." + y)[0] || (v = 0)
								}), 1 == v ? T[0] && (h.children("." + y).addClass(m), h.children("." + y).children("." + s).removeClass(
									C), L.children("." + y).children("." + s).last().addClass(C).removeClass(m)) : d.children("." + y).children(
									"." + s).last().addClass(C), !T.parent("." + y)[0] && T.next()[0] && L.removeClass(C)
					...
					if (r > w) return e.elem.children("." + s).last().children("." + y).addClass(m), e.elem.append(d), S(A), d.prev()
						.children("." + y).children("." + s).last().removeClass(C), d.addClass("layui-tree-setHide"), d.children(
							"." + y).children("." + s).last().addClass(C), a.dragend && a.dragend("拖拽成功，插入最外层节点", d, e.elem), !1
				}
			})
		})
		...
		a.layTool.on("click", "*[lay-event]", function(e) {
			...
			switch (layui.stope(e), F.trigger("table.tool.panel.remove"), l.close(a.tipsIndex), c) {
									...
									layui.each(t, function(t, i) {
										if (e + "-" + t === n) {
											...
											i.hide = !l, a.elem.find('*[data-key="' + o.index + "-" + n + '"]')[l ? "removeClass" :
												"addClass"](f), d != i.hide && a.setParentCol(!l, r), a.resize()
										}
									})
								})
							})
				...
				case "LAYTABLE_PRINT":
					...
					v.append(a.layMain.find("table").html()), v.append(a.layTotal.find("table").html()), v.find(
			...
			encodeURIComponent(function() {
				...
				return layui.each(t, function(t, l) {
					...
					"object" == typeof e ? (layui.each(e, function(e, a) {
						...
						a.field && "normal" == a.type && !a.hide && (0 == t && i.push(a.title || ""), n.push('"' + u(a, l[a.field],
							l, "text") + '"'))
					}), a.push(n.join(","))
				}), i.join(",") + "\r\n" + a.join("\r\n")
